...
from backend.database import placa_robada
import sys
...
class VehicleDetectorUI(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.btn_load.clicked.connect(self.detect_from_image)
    ...
    def detect_from_image(self):
        filename, _ = QFileDialog.getOpenFileName(
            self, "Seleccionar imagen", "", "Imágenes (*.jpg *.png *.jpeg)"
        )

        if filename:
            result = self.detector.detect_vehicle(filename)
            ...
            self.label_image.setPixmap(pixmap.scaled(550, 400, Qt.KeepAspectRatio))

            placa = result["plate_text"]
            estatus = "⚠️ VEHÍCULO ROBADO" if placa_robada(placa) else "✅ VEHÍCULO SEGURO"
            color = "#e74c3c" if "ROBADO" in estatus else "#27ae60"

            html = f"""
            <div style='text-align: center;'>
                <p style='font-size: 18px; margin-bottom: 20px;'>
                    <span style='color: #2c3e50; font-weight: bold;'>Placa:</span> 
                    <span style='color: #1E90FF; font-weight: bold;'>{placa}</span>
                </p>
                <p style='font-size: 20px; margin-bottom: 20px; color: {color}; font-weight: bold;'>
                    {estatus}
                </p>
                <p style='font-size: 16px; color: #7f8c8d;'>
                    Detectado el {result["timestamp"]}
                </p>
            </div>
            """
